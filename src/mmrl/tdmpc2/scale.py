"""Running trimmed scale estimator for TD-MPC2 value normalization."""

import torch


class RunningScale(torch.nn.Module):
    """Running trimmed scale estimator using 5th/95th percentiles."""

    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
        self.value = torch.nn.Buffer(torch.ones(1, dtype=torch.float32))
        self._percentiles = torch.nn.Buffer(torch.tensor([5, 95], dtype=torch.float32))

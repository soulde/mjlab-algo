...
class Ensemble(nn.Module):
    """Vectorized ensemble of modules using torch.vmap."""
...
class ShiftAug(nn.Module):
    """Random shift image augmentation (from DrQv2)."""
...
class SimNorm(nn.Module):
    """Simplicial normalization (from https://arxiv.org/abs/2204.00616)."""
...
def mlp(
    ...
    """Build MLP with LayerNorm, Mish activations, and optional dropout."""
...
def conv(in_shape: tuple[int, ...], num_channels: int, act=None) -> nn.Sequential:
    """Build convolutional encoder for 64x64 RGB observations."""
...
def enc(cfg, out=None):
    """Return a ModuleDict of encoders for each observation key."""
    if out is None:
        out = {}
...
def api_model_conversion(target_state_dict: dict, source_state_dict: dict) -> dict:
    """Convert checkpoint from old API to torch.compile-compatible format."""
    ...
    # Rename keys for Q-network parameters
    for key in list(source_state_dict.keys()):
        if key.startswith("_Qs."):
            ...
            new_state_dict[new_total_key] = val = source_state_dict.pop(key, None)
            ...
            del source_state_dict[key]
            new_state_dict[new_total_key] = source_state_dict.pop(key, None)

    # Add batch_size and device from target
    for prefix in ("_Qs.", "_detach_Qs_", "_target_Qs_"):
        for meta_key in ("__batch_size", "__device"):
            new_key = prefix + "params." + meta_key
            new_state_dict[new_key] = target_state_dict[new_key]

    # Validate that all Q-keys are accounted for

"""Replay buffer for TD-MPC2 training.

Stores episodes as TensorDicts and samples contiguous subsequences
of length ``horizon+1`` for temporal-difference learning.
"""
...
class Buffer:
    """Replay buffer for TD-MPC2.

    Stores episodes and samples random subsequences for training.
    Uses CUDA memory if available, falling back to CPU.
    """
    ...
    def add(self, td: TensorDict) -> int:
        ...
        If the buffer is full, the oldest episode is removed.
        """
    ...
    def _sample_episode(self) -> TensorDict:
        """Sample a random episode that is long enough."""
        min_len = self.cfg.horizon + 1
        # Weight by episode length for uniform transition sampling
    ...
    def sample(
        ...
        # Stack: (batch_size, horizon+1, ...) -> (horizon+1, batch_size, ...)
